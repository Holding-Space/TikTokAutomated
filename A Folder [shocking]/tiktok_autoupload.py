    ...
    while True:
        ...
        try:
            if r.json()["status_msg"] == "You are posting too fast. Take a rest.":
                send_notification("TikTok Notification",f"[-] You are posting too fast, try later again")
                print("[-] You are posting too fast, try later again")
                return False
            uploaded = True
            break
        except:
            if tryingtoupload == False:
                tryingtoupload = True
                print("[-] Attempting to upload video...")
                send_notification("TikTok Notification","[-] Attempting to upload video...") 
    if not uploaded:
        send_notification("TikTok Notification",f"[-] Could not upload video")
    ...
    for j in r.json()["infos"]:
        if j["creationID"] == creation_id:
            if j["tasks"][0]["status_msg"] == "Y project task init":
                send_notification("TikTok Notification","[+] Video got uploaded!")
                print("[+] Video got uploaded!")
                return True
            send_notification("TikTok Notification",f"[-] Video could not be uploaded: {j['tasks'][0]['status_msg']}")
